...
class Config
{
    ...
    protected function validate()
    {
        ...
        if (!array_key_exists(self::KEY_CACHE_TIME, $this->configData) || !is_numeric($this->configData[self::KEY_CACHE_TIME])) {

            $this->setDefault(self::KEY_CACHE_TIME, self::DEFAULT_CACHE_TIME);
        }

        if (!array_key_exists(self::KEY_USER, $this->configData) || !is_string($this->configData[self::KEY_USER])) {

            $this->throwInvalidConfigDataException(self::KEY_USER);
        }

        if (!array_key_exists(self::KEY_USE_FILE_CACHE, $this->configData) || !is_bool($this->configData[self::KEY_USE_FILE_CACHE])) {

            $this->setDefault(self::KEY_USE_FILE_CACHE, self::DEFAULT_USE_FILE_CACHE);

            if ($this->configData[self::KEY_USE_FILE_CACHE] === true) {

                if (!array_key_exists(self::KEY_FILE_CACHE_PATH, $this->configData) || !is_string($this->configData[self::KEY_FILE_CACHE_PATH])) {
